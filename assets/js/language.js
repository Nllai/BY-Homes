function toggleLanguage() {
    document.getElementById("chinese").classList.toggle('on');
    document.getElementById("english").classList.toggle('on');
    document.getElementById("english-icon-index").classList.toggle('on');
    document.getElementById("chinese-icon-index").classList.toggle('on');
    document.getElementById("english-icon-about").classList.toggle('on');
    document.getElementById("chinese-icon-about").classList.toggle('on');
    document.getElementById("chinese").classList.toggle('off');
    document.getElementById("english").classList.toggle('off');
    document.getElementById("english-icon-index").classList.toggle('off');
    document.getElementById("chinese-icon-index").classList.toggle('off');
    document.getElementById("english-icon-about").classList.toggle('off');
    document.getElementById("chinese-icon-about").classList.toggle('off');
}
